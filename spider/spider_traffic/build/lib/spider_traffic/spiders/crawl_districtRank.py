...
from spider_traffic.items import SpiderDistrictItem


class CrawlDistrictrankSpider(scrapy.Spider):
    name = 'crawl_districtRank'
    ...
    def start_requests(self):
        ...
        for i in self.pd_codes:
            ...
            for dataType in [1, 3, 4]:
                url = base_url + f'?cityCode={code}&linksType={dataType}'
                yield Request(url,
                              callback=self.parse,
    ...
    def parse(self, response):
        ...
        for i in json_datas:
            ...
            index = i.get('index')
            speed = i.get('speed')
            temp.append([pid, number, name, index, speed])
...
if __name__ == '__main__':
    cmdline.execute("scrapy crawl crawl_districtRank --nolog".split())
